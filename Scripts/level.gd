...
func spawn_premade_tiles(lvl_obj, overwrite_details=true, preset_course=false):
	...
	reset_level_vals()
	spawn_player()
	var dif = meta.set_difficulty()
	meta.spawn_enemies()
	meta.set_turn_order_info()
	get_node("/root/player/cam_body/cl/card/dif_dist").set_text(str(dif))
...
func spawn_player(new_player=true):
	if new_player:
		...
		meta.map_char(p.char_type, p, true)
...
func handle_level_start_camera_change_display():
	...
	if get_node("/root").has_node("player"):
		var p = get_node("/root/player")
		if main.checkIfNodeDeleted(p) == false and p and p.current_tile:
...
func handle_turn_order_display():
	if not get_node("/root").has_node("player") or main.checkIfNodeDeleted(round_turn_icons) == false and round_turn_icons:
		return
	var p = get_node("/root/player")
	var char_count = 0
	var x_buffer = 132
	for character_icon in round_turn_icons:
		if not character_icon or main.checkIfNodeDeleted(character_icon) == true:
			continue
		char_count += 1
...
func update_turn_order_details():
	var index = 0
	for icon in round_turn_icons:
		if not icon or main.checkIfNodeDeleted(icon) == true:
			continue
		icon.visible = false
